...
macro_rules! impl_elastic_array {
	($name: ident, $dummy: ident, $size: expr) => (
		...
		impl<T> $name<T> where T: Copy {
			pub fn new() -> $name<T> {
				$name {
					raw: $dummy::Arr(unsafe { ::core::mem::uninitialized() }),
					len: 0
				}
			}
			...
			pub fn push(&mut self, e: T) {
				match self.raw {
					...
					$dummy::Arr(_) => {
						...
						unsafe {
							::core::ptr::copy(self.raw.slice().as_ptr(), vec.as_mut_ptr(), self.len);
			...
			pub fn clear(&mut self) {
				self.raw = $dummy::Arr(unsafe { ::core::mem::uninitialized() });
				self.len = 0;
			}
			...
			pub fn into_vec(self) -> Vec<T> {
				match self.raw {
					$dummy::Arr(a) => {
						...
						unsafe {
							::core::ptr::copy(a.as_ptr(), vec.as_mut_ptr(), self.len);
							vec.set_len(self.len);
						}
						vec
					}
