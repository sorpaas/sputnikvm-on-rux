...
use super::common::{BLOCKS_RLP_SWAPPER, SNAPSHOT_RLP_SWAPPER};
use super::{UntrustedRlp, Compressible, encode, RlpStream};

/// Stores RLPs used for compression
pub struct InvalidRlpSwapper<'a> {
	invalid_to_valid: BTreeMap<&'a [u8], &'a [u8]>,
	valid_to_invalid: BTreeMap<&'a [u8], &'a [u8]>,
}
...
impl<'a> InvalidRlpSwapper<'a> {
	...
	pub fn new(rlps_to_swap: &[&'a [u8]], invalid_rlps: &[&'a [u8]]) -> Self {
		...
		let mut invalid_to_valid = BTreeMap::new();
		let mut valid_to_invalid = BTreeMap::new();
